...
//   const { data, isLoading } = useSearchChatsQuery(searchQuery ? { query: searchQuery, email: userData?.email } : undefined);
...
export default function SearchModal({ onClose }: SearchModalProps) {
  ...
  // Fetch chats based on search query
  const { data, isLoading, error } = useSearchChatsQuery(
    searchQuery ? { query: searchQuery, email: userData?.email } : undefined,
    { skip: !searchQuery || !userData?.email } // Skip query if no search term or email
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="max-h-96 overflow-y-auto scrollbar-hide pt-4">
          <h3 className="px-4 py-2 text-sm font-medium text-white">Today</h3>
          {isLoading && <p className="px-4 text-white">Loading...</p>}
          {error && (
            <p className="px-4 text-red-500">
              Error fetching chats: {JSON.stringify(error)}
            </p>
          )}
          {!isLoading && !error && (
            <ul>
              {(data as SearchResponse)?.results?.length > 0 ? (
                (data as SearchResponse).results.map((chat: Chat) => (
                  <Link
                    onClick={() => handleSessionSelect(chat.session_id)}
                    key={chat.session_id}
                    href={`/chat/${chat.session_id}`}
                ...
                <p className="px-4 text-white">No chats found</p>
              )}
            </ul>
          )}
        </div>
